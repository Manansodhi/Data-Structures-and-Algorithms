...
Given a wooden stick of length n units. The stick is labelled from 0 to n. For example, a stick  of length 
6 is labelled as follows:
...
public:
  ...
  //TABULATION
    int minCost(int n, vector<int>& cuts) {
      ...
      for(int i=m;i>=1;i--){
        for(int j=1;j<=m;j++){
          if(i>j)continue; //base condn m kuch nahi karta dp[][] phele se hi 0 intialized h
          int mini = 1e9;
          for(int idx=i;idx<=j;idx++){
